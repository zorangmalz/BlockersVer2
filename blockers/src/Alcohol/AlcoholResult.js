...
export default function AlcoholResult({navigation,route}) {
    ...
    useEffect(()=>{
        ...
        if(result>=35){
            setResults("Bad")
            setResultcontent( "알코올 남용이나 의존 단계입니다. 음주량과 음주횟수 조절이 어려운 상태입니다. 술을 마셔야 기분도 좋고 일도 잘되고 관계도 좋아진다고 생각합니다. 술을 줄이는 단계가 아니라 끊어야 합니다. \n● 신체 질환이나 사회적 역할에 어려움이 있을 것입니다. 예) 직장, 가정, 지역사회에서 술로 인한 사회적 혹은 법적 문제 유발(음주운전이나 가정폭력 등) 전문 병/의원이나 알코올상담센터 혹은 정신보건센터에 연계하여 진단과 치료를 받도록 합니다. 신체에 질병이 생기면 치료받아야 나을 수 있는 것처럼 알코올 사용 장애도 치료가 필요한 질병입니다")
        }else if(15<= result){
            setResults("Normal")
