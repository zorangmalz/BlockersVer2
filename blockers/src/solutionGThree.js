...
export default function SolutionGThree({ navigation,route }) {
    ...
    const pushone = () => {
        ...
        setTimeout(() => {
            firestore().collection("UserInfo").doc(UID).collection("Solution").doc("sg").set({
                answer:sg+1
            })
            navigation.navigate('SolutionResult',{UID:UID});
        }, 200)
    }
    ...
    return (
        <>
            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
                ...
                <ScrollView>
                    <Text style={solution.largeText}>당신은 친구들과 회식하거나 모임을 할 때 항상 흡연하시나요?</Text>
                    {one === false ?
